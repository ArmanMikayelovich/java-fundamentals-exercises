...
public class CrazyOptionals {
    ...
    public static Optional<Account> retrieveAccountGmail(AccountProvider accountProvider) {
        return accountProvider.getAccount()
                .filter(account -> account.getEmail().endsWith("@gmail.com"));
    }
     ...
     * then {@link NoSuchElementException} should be thrown.
     *
     * @param accountProvider
    ...
    /**
     * Returns an {@link Accounts} with the highest balance. Throws {@link NoSuchElementException} if input
     * list is empty
     *
     * @param accounts
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream()
                .mapToDouble(account -> account.getCreditBalance().get().doubleValue())
                .sum();
    }
}
