...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(acc -> Sex.MALE.equals(acc.getSex())));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(acc -> {
            String[] split = acc.getEmail().split("@");
            return split[1];
        }));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().map(acc -> acc.getFirstName() + acc.getLastName())
                .reduce("", (x, y) -> (x + y).replace(" ", "")).length();
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream().sorted(Comparator.comparing(acc -> acc.getFirstName() + acc.getLastName()))
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(acc -> acc.getEmail().contains(emailDomain));
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream().filter(acc -> acc.getEmail().equals(email))
                .findAny()
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream().collect(Collectors.toMap(Account::getId, acc -> acc));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream().collect(Collectors.groupingBy(Account::getLastName,
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream().collect(
                Collectors.groupingBy(acc -> acc.getCreationDate().getMonth(),
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                .flatMap(account -> {
                    Stream<Character> firstNameChars = account.getFirstName().length() >= nameLengthBound
                            ? account.getFirstName().toLowerCase().chars().mapToObj(c -> (char) c)
                            : Stream.empty();

                    Stream<Character> lastNameChars = account.getLastName().length() >= nameLengthBound
                            ? account.getLastName().toLowerCase().chars().mapToObj(c -> (char) c)
                            : Stream.empty();

                    return Stream.concat(firstNameChars, lastNameChars);
                })
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }
